...
| `get_a_trade_universe` | **可交易域**：全市场按 total 分页拉取，剔除 ST/退市、停牌、次新（默认 60 天）、可选北交所与成交额下限；返回逐项淘汰计数（可审计）。东财 clist 502/截断时自动回退「本地代码表 + 腾讯批量行情」，输出带 `source` 与 `notes` |
| `get_a_market_snapshot` | **全市场宽度**：涨跌家数、涨跌幅分布桶、中位数/均值、总成交额（冷调用约 1 分钟，之后走缓存）。同样带东财→腾讯回退 |
